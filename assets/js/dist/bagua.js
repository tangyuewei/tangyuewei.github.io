...
//根据六爻生成主卦
function getHexagramFromBinary(binary) {
  if (binary.length !== 6) return null;
  return hexagrams[binary];
}
//动爻变卦逻辑（支持多动爻）
function changeBinaryAt(binary, index) {
  const arr = binary.split('');
  arr[index] = arr[index] === '0' ? '1' : '0';
  return arr.join('');
}

function getChangingHexagram(binary, ...positions) {
  let newBinary = binary;
  positions.forEach(pos => {
    newBinary = changeBinaryAt(newBinary, pos);
  });
  return getHexagramFromBinary(newBinary);
}
//图形化显示卦象（符号表示）
const yaoMap = {
  '0': '--', // 阴爻
...
function displayGua(binary) {
  const lines = binary.split('').reverse().map(bit => yaoMap[bit]);
